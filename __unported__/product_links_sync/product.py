...
class product_product(Model):
    ...
    def _update_product_link_last_date(self, cr, uid, vals, context=None):
        if 'product_link_ids' in vals:
            vals['product_link_write_last_date'] = datetime.now().strftime(
                DEFAULT_SERVER_DATETIME_FORMAT)
        return vals

    def create(self, cr, uid, vals, context=None):
        vals = self._update_product_link_last_date(
    ...
    def get_field_to_export(self, cr, uid, ids, mapping, mapping_id, context=None):
        fields_to_read = super(product_product, self).get_field_to_export(
            cr, uid, ids, mapping, mapping_id, context=context)
        if context.get('export_product') == 'link':
            fields_to_read = ['product_link_ids']
        else:
            if 'product_link_ids' in fields_to_read:
                fields_to_read.remove('product_link_ids')
        ...
        if 'is_active' in vals and not context.get('product_link_date_updated'):
            for link in self.browse(cr, uid, ids, context=context):
                link.product_id.write(
                    {'product_link_write_last_date': datetime.now().strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT)},
                    context=context)
...
class product_export_wizard(TransientModel):
    ...
    def _export_one_product(self, cr, uid, external_session, product_id, options, context=None):
        res = super(product_export_wizard, self)._export_one_product(
            cr, uid, external_session, product_id, options, context=context)
        if 'export_link' in options:
            context['export_product'] = 'link'
            self.pool.get('product.product')._export_one_resource(
                cr, uid, external_session, product_id, context=context)
        return res

    def _get_all_options(self, cr, uid, context=None):
        res = super(product_export_wizard, self)._get_all_options(
