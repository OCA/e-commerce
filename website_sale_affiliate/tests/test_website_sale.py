...
class WebsiteSaleCase(HttpCase, SaleCase):
    ...
    def test_shop(self):
        ...
        try:
            self.url_open(f"/shops?aff_ref={str(self.demo_affiliate.id)}")
    ...
    def test_product(self):
        ...
        try:
            self.url_open(
                f"{self.demo_product.website_url}?aff_ref={self.demo_affiliate.id}"
            )
            session = http.root.session_store.get(self.session_id)
            self.assertEqual(
                session.get("affiliate_request"),
    ...
    @mock.patch(f"{CONTROLLER_PATH}.request")
