...
class TestRestrictPartnerProduct(HttpCase):
    ...
    def test_restrict_products_param_enabled(self):
        ...
        self.portal_partner.invalidate_recordset()
        self.assertTrue(
    ...
    def test_admin_user_see_all_products(self):
        # Test that admin users with internal group can see products
        # even without the restrict parameter enabled
        admin_user = self.env.ref("base.user_admin")

        # Verify admin is in internal group
        self.assertTrue(
            admin_user.has_group("base.group_user"),
            "Admin should be in internal user group",
        )

        # Verify restrict_products_for_all_users is False for internal users
        # when parameter is not set
        self.env["ir.config_parameter"].sudo().set_param(
            "website_sale_restrict_by_pricelist.restrict_products", ""
        )
        admin_partner = admin_user.partner_id
        admin_partner.invalidate_recordset()
        self.assertFalse(
            bool(admin_partner.restrict_products_for_all_users),
            "Internal users should not have product restrictions",
        )
