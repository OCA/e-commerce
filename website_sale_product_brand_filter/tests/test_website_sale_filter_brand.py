...
from odoo.addons.website_sale_product_brand_filter.controllers.website_sale import (
    Website,
)
...
class TestWebsiteSaleFilterBrandHttpCase(HttpCase):
    def setUp(self):
        ...
        self.env.ref(
            "website_sale_product_brand_filter.website_sale_filter_brand_products_brands"
        ).active = True

    def test_ui_website_admin(self):
        """Test frontend tour."""
        self.start_tour("/shop", "website_sale_product_brand_filter", login="admin")

    def test_ui_website_portal(self):
        """Test frontend tour."""
        self.start_tour("/shop", "website_sale_product_brand_filter", login="portal")
...
class WebsiteSale(SavepointCase):
    ...
    def test_brands_domain(self):
        ...
        with MockRequest(
            ...
            res11 = self.WebsiteSaleController._update_domain(
                brands_list, simple_domain
            )
            res2 = self.WebsiteSaleController._get_brands([])
        ...
        self.assertEqual(res11, required_domain2, "Must be the same")
        self.assertEqual(res2.ids, brand_ids.ids, "Must be the same")
