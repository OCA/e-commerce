...
patch(CartService.prototype, {
     ...
     * keys of the main product, so the variants filled in the matrix can't be
     * added to the cart request from there. `additionalData` is instead spread
     * as is, both in the props of the dialog and in the request built when the
    ...
    _openProductConfigurator(
        ...
        return super._openProductConfigurator(
            ...
            productCustomAttributeValues,
            options,
            {...additionalData, matrixSelection: {}}
        );
    },
     ...
     * @override
     */
    _makeRequest({matrixSelection, ...params}) {
        return super._makeRequest({...params, ...matrixSelection});
    },
});
