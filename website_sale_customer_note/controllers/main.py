...
class WebsiteSale(website_sale):
    def checkout_values(self, data=None):
        ...
        if result and data:
            ...
            result['checkout']['shipping_note'] = data.get('shipping_note', False)
    ...
    def checkout_form_save(self, checkout):
        ...
        if 'note' in checkout:
            ...
            order.partner_id.comment = checkout['note']

        if 'shipping_note' in checkout:
            order = request.website.sale_get_order(
                force_create=1, context=request.context)
            order.partner_shipping_id.comment = checkout['shipping_note']
